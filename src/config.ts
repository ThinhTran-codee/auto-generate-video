...
export function loadConfig(): Config {
  const rawProvider = (process.env.TTS_PROVIDER ?? 'edge-tts')
    .trim()
    .toLowerCase();
  const provider = (
    rawProvider === 'edgetts' ? 'edge-tts' : rawProvider
  ) as TtsProvider;
  ...
  if (provider === 'lucylab') {
    if (
      !process.env.VIETNAMESE_API_KEY ||
      process.env.VIETNAMESE_API_KEY.trim() === ''
    ) {
      throw new Error(
    ...
    if (
      !process.env.VIETNAMESE_VOICEID ||
      process.env.VIETNAMESE_VOICEID.trim() === ''
    ) {
      throw new Error(
    ...
    if (
      !process.env.VBEE_ACCESS_TOKEN ||
      process.env.VBEE_ACCESS_TOKEN.trim() === ''
    ) {
      throw new Error(
  ...
  if (videoTheme !== 'dark-neon' && videoTheme !== 'light-pro') {
    throw new Error(
      `VIDEO_THEME must be "dark-neon" or "light-pro", got "${videoTheme}"`,
    );
  }
  ...
  return {
    ...
    lucylabEndpoint:
      process.env.LUCYLAB_ENDPOINT ?? 'https://api.lucylab.io/json-rpc',
    ...
    elevenlabsEndpoint:
      process.env.ELEVENLABS_ENDPOINT ?? 'https://api.elevenlabs.io/v1',
    ...
    vbeeVoiceCode:
      process.env.VBEE_VOICE_CODE ?? 'n_hanoi_male_protrainer_education_vc',
